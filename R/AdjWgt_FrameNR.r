...
#'   
#' @description Using the \code{evalstatus} field in the design file, adjust the design
...
#' 
#' @param popn	Takes the value of \code{"Target"} or \code{"Sampled"} to reflect inference to either the original
...
#' population of interest after accounting for frame error (\code{popn = "Target"}) or frame and nonresponse
#' error (\code{popn = "Sampled"}). 
#'   
...
# Error check evalstatus field
	...
	if(any(!dat[, evalstatus]%in%EvalVector)) print(paste("The evalstatus field should take only the following values:","Target - Surveyed,Target - Not Surveyed,Not Evaluated,OverSamp,Non-Target"))
